...
contract IotDevice{
    ...
    function createIotDevice(string memory _newName,address _newIotDevice) public {
        iotdevice.push(Iot(CurrentIotId, _newName, '', 0, 0,0));
    ...
    iotdevice[_IotId].time = block.timestamp;
    ...
    function getTime(uint _IotId) public view  returns (uint) {
    return iotdevice[_IotId].time;
    }
