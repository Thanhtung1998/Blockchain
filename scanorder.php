...
<html lang="en">
  ...
  <?php
    if( $_SESSION['role']==2){   // cho phep role co gia tri 0 va 1 truy cap
  ?>
        ...
        <div class="mycardstyle">
            <div class="greyarea">
                ...
                <form id="form2" autocomplete="off">
                    ...
                    <div class="formitem">
                        <label type="text" class="formlabel"> Cancellation is only valid for the first 24 hours from the date of order </label>
                        <input type="text" class="forminput" id="prodCancel" placeholder="Reason for Cancellation " required>
                        <input type="hidden" class="forminput" id="user" value=<?php echo $_SESSION['username']; ?> required>
                    </div>
                    <button class="formbtn" id="mansub" type="submit">Confirm</button>
                </form>
            </div>
      </center>
    <?php
		...
		"inputs": [
			{
				...
				"name": "index",
				"type": "uint256"}],"name": "Added","type": "event"},{"constant": false,"inputs": [{"internalType": "uint256","name": "_productId","type": "uint256"},{"internalType": "string","name": "info","type": "string"}],"name": "addState","outputs": [{"internalType": "string","name": "","type": "string"}],"payable": false,"stateMutability": "nonpayable","type": "function"},{"constant": false,"inputs": [{"internalType": "string","name": "_a","type": "string"},{"internalType": "string","name": "_b","type": "string"}],"name": "concat","outputs": [{"internalType": "string","name": "","type": "string"}],"payable": false,"stateMutability": "nonpayable","type": "function"},{"constant": false,"inputs": [{"internalType": "string","name": "_text","type": "string"},{"internalType": "string","name": "_date","type": "string"}],"name": "newItem","outputs": [{"internalType": "bool","name": "","type": "bool"}],"payable": false,"stateMutability": "nonpayable","type": "function"},{"constant": false,"inputs": [{"internalType": "uint256","name": "_productId","type": "uint256"}],"name": "searchProduct","outputs": [{"internalType": "string","name": "","type": "string"}],"payable": false,"stateMutability": "nonpayable","type": "function"}];

var Address ='0xcB644F840035BC997118FBa4fEa2811df9B51d9f';
    </script>
    <!-- <script src="./build/contracts/Supplychain.json"></script> -->
    ...
    <script>
      ...
      const order = new web3.eth.Contract(ABI, Address);
      // this.setState({supplychain: supplychain, order: order})
    // var contract = new web3.eth.Contract(contractAbi, contractAddress);
    ...
    $('#form1').on('submit', function(event) {
        ...
        web3.eth.getAccounts().then(async function(accounts) {
          var receipt = await supplychain.methods.newItem(prodname, thisdate).send({ from: accounts[0], gas: 1000000 })
          .then(receipt => {
    ...
    $('#form2').on('submit', function(event) {
        ...
        prodid = $('#prodid').val();
        username = $('#user').val(); 
        prodCancel =$('#prodCancel').val();
        console.log(prodid);
        var today = new Date();
        var thisdate = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate()+ ' ' +'Time: '+today.getHours()+':'+today.getMinutes()+':'+today.getSeconds();
        var info = "<br><br><b>Date: "+thisdate+ "</b><br>Reason: "+ prodCancel +"<br> Your order has been canceled with "+ username ;
        web3.eth.getAccounts().then(async function(accounts) {
          var receipt = await supplychain.methods.addState(prodid, info).send({ from: accounts[0], gas: 1000000 })
          .then(receipt => {
              var msg="Your order has been canceled successfully ";
              $("#alertText").html(msg);
              $("#qrious").hide();
              $("#bottomText").hide();
              $(".customalert").show("fast","linear");
          });
          var receipt1 = await order.methods.addState(prodid, info).send({ from: accounts[0], gas: 1000000 })
          .then(receipt1 => {
              var msg="Your order has been canceled successfully ";
              $("#alertText").html(msg);
              $("#qrious").hide();
